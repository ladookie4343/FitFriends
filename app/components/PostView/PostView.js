...
export default class PostView extends Component {
  ...
  async componentWillMount () {
    const result = await getAsyncStorage(this.props.navigation.state.params.postID);
  ...
  async _getPostDetails () {
    await getPostDetails(this.props.navigation.state.params.postID, (result) => {
      this._savePostDetailsToState(result);
      setAsyncStorage(this.props.navigation.state.params.postID, result);
    });
  }
  ...
  _renderCreatedString () {
    return 'Posted ' + getDateTimeString(this.props.navigation.state.params.createdTime);
  }
  ...
  _renderMessageString () {
    return this.props.navigation.state.params.message
  }

  _renderStoryString () {
    if (this.props.navigation.state.params.story) {
      return (
  ...
  _renderWebView (url) {
    this.props.navigation.navigate('WebViewWrapper', { url });
  }
